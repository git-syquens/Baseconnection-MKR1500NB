...
static void ensureRATMode(int rat) {
  ...
  while (true) {
    ...
    bool correctSetting = (rat == 7) ? (urat.indexOf("+URAT: 7") >= 0) : (urat.indexOf("+URAT: 8") >= 0);

    if (!correctSetting) {
      Serial.print("Setting URAT="); Serial.print(rat);
    ...
    if (cops.indexOf(String(",") + rat) >= 0) {
      Serial.print("RAT "); Serial.print(rat);
      Serial.println(" confirmed by network.");
      if (rat == 7) {
        Serial.print("Locked to LTE-M with APN: "); Serial.println(APN);
      }
      break;
    }

    Serial.println("Network still not reporting requested RAT; cycling modem again...");
    sendAT("AT+CFUN=0", 2000);
    String cmd2 = String("AT+URAT=") + rat;
    sendAT(cmd2.c_str(), 3000);
    sendAT("AT+CFUN=1,1", 2000);
    delay(6000);
  }
